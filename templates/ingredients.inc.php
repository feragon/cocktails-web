	...
	if(array_key_exists($categorie, $Hierarchie)) {
		if(array_key_exists('sous-categorie', $Hierarchie[$categorie])) {
			...
			foreach($Hierarchie[$categorie]['sous-categorie'] as $key => $value) {
			    $url_key = str_replace(" ", "%20", $value);
				echo '<li><a href="?R=Ingredients&C=', $url_key, '"><i class="fa fa-tint fa-fw"></i>', $value, '</a></li>';
			}
			
			echo '<li class="flex-line-breaker"></li>';
		}
		
		foreach($Recettes as $key => $recette) {
			foreach($recette['index'] as $ingredient) {
				if($ingredient == $categorie)
					echo '<li class="li_cocktail"><a href="?R=Cocktail&K=',$key, '"><i class="fa fa-glass fa-fw"></i>', $recette['titre'], '</a></li>';
			}
		}
	}
    ?>
